...
values(
  ...
  'Road to IELTS 8.0',
  'Lộ trình IELTS 8.0',
  '100-level gamified English programme',
  'Chương trình tiếng Anh game hóa 100 level'
)
on conflict(slug) do nothing;

insert into public.lessons(
  course_id,level_number,slug,title,title_vi,subtitle,subtitle_vi,
  stage_number,stage_name,cefr_level,xp_reward,coin_reward,
  estimated_minutes,is_checkpoint,is_published
)
select
  (select id from public.courses where slug='road-to-ielts-8'),
  n,
  'level-'||n,
  ...
  case
    ...
    when n<=50 then 'IELTS Core'
    when n<=60 then 'Academic Development'
    when n<=70 then 'Confident IELTS'
    when n<=80 then 'Advanced IELTS'
    when n<=90 then 'Band 7.5'
    else 'Road to Band 8'
  end,
  case
